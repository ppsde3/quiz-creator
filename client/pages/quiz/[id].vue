...
<template>
    <div>
        ...
        <div v-if="quizScore > -1" class="bg-emerald-300/70 p-3 text-center font-semibold rounded-lg">
            You answered {{ quizScore }}/{{ quizQuestions.length }} questions correctly
        </div>
        <div v-else>
                ...
                <div v-for="(question, index1) in quizQuestions" class="">
                    <div class="p-2 my-5">
                        <div class="">
                            ...
                            <div
                                class="mb-1 py-1 px-10 bg-orange-400/40 font-light text-sm text-black/80 rounded-md rounded-t-none"
                            >
                                {{
                        ...
                        <div v-if="question.typeOfQuestion == 1">
                            <div v-for="(option, index2) in question.options">
                                <div class="my-1 flex flex-row rounded-lg">
                                    <input
                                        type="radio"
                                        :id="'type1-' + index1 + '-' + index2"
                                        :value="option"
                                        class="appearance-none"
                                        v-model="markedAnswers[index1][0]"
                                    />
                                    <label
                                        :for="'type1-' + index1 + '-' + index2"
                                        class="w-full inline-block py-2 pl-5 rounded-lg cursor-pointer"
                                        :class="
                                            markedAnswers[index1][0] == option
                        ...
                        <div v-else-if="question.typeOfQuestion == 2">
                            <div v-for="(option, index2) in question.options">
                                <div class="my-1 flex flex-row rounded-lg">
                                    <input
                                        type="checkbox"
                                        :id="'type2-' + index1 + '-' + index2"
                                        :value="option"
                                        class="appearance-none"
                                        v-model="markedAnswers[index1]"
                                    />
                                    <label
                                        :for="'type2-' + index1 + '-' + index2"
                                        class="w-full inline-block py-2 pl-5 cursor-pointer rounded-lg"
                                        :class="
                                            markedAnswers[index1].includes(option)
                                                ? 'bg-green-300 hover:bg-green-400'
                                                : 'bg-gray-300 hover:bg-gray-400'
                                        "
                                        >{{ option }}</label
